...
class AdminUsersStudentsController extends Controller
{
    public function usersGet(HttpRequest $request): HttpResponse
    {
        ...
        $users = $this->userMapper->fetchWithRole(1);
        $data = [];
        ...
        $view = $this->render("AdminUsersStudentsPage", ["error" => $error, "message" => $message, "data" => $data]);
